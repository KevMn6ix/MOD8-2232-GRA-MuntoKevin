...
router.post('/auth/login', credentialsParser, async (req, res, next) => {
  try {
    ...
    const pageCountMinutes = Number.parseInt(process.env.SESSION_DURATION) || 15 // Extend session by pageCount specified in .env
    const session = await createSession(credentials.username, pageCountMinutes)
...
const createSession = async (username, pageCountMinutes) => {
  const id = randomUUID().toString()
  const startTime = new Date(Date.now())
  const expiryTime = new Date(startTime.getTime() + pageCountMinutes * 60 * 1000)
